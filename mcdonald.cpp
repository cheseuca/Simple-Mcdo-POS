    ...
    , ui(new Ui::Mcdonald)
{
    ...
    db.setDatabaseName(
        "./mcdonalds_product_price.db"); //Need to be hard coded to work, use the actual location of the db file in your computer
...
void Mcdonald::on_pay_clicked()
{
    ...
    if (bigmacqty > 0) {
        ...
        summary += "Big Mac x " + QString::number(bigmacqty) + " = PHP "
                   + QString::number(bigmacSubtotal, 'f', 2) + "\n";
    }
    if (qpounderqty > 0) {
        double quarterpoundSubtotal = getProductSubtotal("Quarter Pounder", qpounderqty);
        summary += "Quarter Pounder x " + QString::number(qpounderqty) + " = PHP "
                   + QString::number(quarterpoundSubtotal, 'f', 2) + "\n";
    }
    if (dpounderqty > 0) {
        double doublepoundSubtotal = getProductSubtotal("Double Pounder", dpounderqty);
        summary += "Double Pounder x " + QString::number(dpounderqty) + " = PHP "
                   + QString::number(doublepoundSubtotal, 'f', 2) + "\n";
    }
    if (cburgerqty > 0) {
        double cheesburgerSubtotal = getProductSubtotal("CheeseBurger", cburgerqty);
        summary += "CheeseBurger x " + QString::number(cburgerqty) + " = PHP "
                   + QString::number(cheesburgerSubtotal, 'f', 2) + "\n";
    }
    if (esausageqty > 0) {
        double eggsausageSubtotal = getProductSubtotal("Egg Sausage", esausageqty);
        summary += "Egg Sausage x " + QString::number(esausageqty) + " = PHP "
                   + QString::number(eggsausageSubtotal, 'f', 2) + "\n";
    }
    if (hamdesalqty > 0) {
        double hamdesalSubtotal = getProductSubtotal("Hamdesal", hamdesalqty);
        summary += "Hamdesal x " + QString::number(hamdesalqty) + " = PHP "
                   + QString::number(hamdesalSubtotal, 'f', 2) + "\n";
    }
    if (pancakeqty > 0) {
        double pancakeSubtotal = getProductSubtotal("Pancake", pancakeqty);
        summary += "Pancake x " + QString::number(pancakeqty) + " = PHP "
                   + QString::number(pancakeSubtotal, 'f', 2) + "\n";
    }
    if (softdrinkqty > 0) {
        double softdrinkSubtotal = getProductSubtotal("Softdrinks", softdrinkqty);
        summary += "Softdrink x " + QString::number(softdrinkqty) + " = PHP "
                   + QString::number(softdrinkSubtotal, 'f', 2) + "\n";
    }
    if (friesqty > 0) {
        double friesSubtotal = getProductSubtotal("French Fries", friesqty);
        summary += "French Fries x " + QString::number(friesqty) + " = PHP "
                   + QString::number(friesSubtotal, 'f', 2) + "\n";
    }
    if (sundaeqty > 0) {
        double sundaeSubtotal = getProductSubtotal("Hot Fudge", sundaeqty);
        summary += "Hot Fudge x " + QString::number(sundaeqty) + " = PHP "
                   + QString::number(sundaeSubtotal, 'f', 2) + "\n";
    }
